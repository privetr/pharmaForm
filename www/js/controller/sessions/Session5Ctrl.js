(function() {
	 ...
	 * Page : seance_5.html
	 * Goal : Managing seance 5
	 */

	.controller('Session5Ctrl', Session5Ctrl);
	
	Session5Ctrl.$inject = ['$state', '$scope', '$stateParams',
                                'pfLocalForageService', 'pfUtilsService', 'pfLookUpService',
                             '$ionicPopup', '$ionicLoading', '$localForage', '$ionicSlideBoxDelegate'];

	/* @ngInject */
	function Session5Ctrl($state, $scope, $stateParams, pfLocalForageService, pfUtilsService, pfLookUpService,
                            $ionicPopup, $ionicLoading, $localForage, $ionicSlideBoxDelegate) {
		...
		vm.saveSession = function (){
            ...
            .then(function() {
                pfUtilsService.showAlert('Séance 5 terminée', 'La séance 5 est désormais terminée, Félicitations !');
